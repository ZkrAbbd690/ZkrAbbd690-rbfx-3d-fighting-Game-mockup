...
int main(int argc, char** argv)
{
    ...
    // ================================================================
    //  CHARACTER (Refactored for firing-Idle/test_out/test.gltf.d)
    // ================================================================
    auto* characterNode = scene->CreateChild("Character");
    
    // REFACTORED: Paths updated to match your active tree directory layout
    auto* prefab = cache->GetResource<PrefabResource>("firing-Idle/test_out/test.gltf.d/Prefab.prefab");
    ...
    if (prefab)
    {
        ...
        if (animCtrl)
        {
            // REFACTORED: Updated internal animation directory target
            const ea::string animPath = "firing-Idle/test_out/test.gltf.d/Animations/mixamo.com.ani";
            if (animCtrl->Play(animPath, 0, true, 0.0f))
            {
                animCtrl->SetSpeed(animPath, 0.5f); // Default baseline speed
    ...
    {
        URHO3D_LOGERROR("Could not find firing-Idle/test_out/test.gltf.d/Prefab.prefab!");
        engine->Exit();
        return 2;
    }
